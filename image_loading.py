...
# ---------------------------- Imports -------------------------------------- #
import numpy as np
# careful: even if you install the "pillow" module, you still need to import
# PIL. Lmao
...
# Open image
img = cv2.imread("debug\\img\\09_060.jpg")
# Prints the array
print(img.shape)
...
# Filtering notes
# A medianBlur filter AFTER thresholding results in disaster, even at the lower
# size.
# A medianBlur filter before thresholding gives slightly better results, but
# still unsatisfactory (fragmented edges).
...
# Running the image through Waifu2x (an online noise removal thing used by CLs)
# then thresholding also gave pretty much the same result

# I just skipped this part for now, probably we can do better by changing those
# parameters
# However, when we have bubbles with edges only 1-2 pixels thick, it might be
# impossible to preserve them and still remove a significant amount of noise..?


# ----------- Preprocessing 1: Thresholding --------------------------------- #

# Thresholding the image. Note: the first return value is used to determine the
...
# There's someone working on an implementation of the algorithm in python here:
